...
pub trait EntityIterator<A: Archetype> {
    ...
    type EntitiesWith<'t, T: Accessor<A> + 't>: Iterator<Item = T::Entity<'t>>
    where
        Self: 't;
    ...
    type ChunksWith<'t, T: accessor::Chunked<A> + 't>: Iterator<Item = T::Chunk<'t>>
    where
        Self: 't;
    ...
    <R as ops::Deref>::Target: ealloc::Shard<Raw = A::RawEntity>,
{
    ...
    type EntitiesWith<'t, T: Accessor<A> + 't> = impl Iterator<Item = T::Entity<'t>>
    where
        Self: 't;
    fn entities_with<T: Accessor<A>>(&self, mut accessor: T) -> Self::EntitiesWith<'_, T> {
        ...
        self.entities().map(move |entity| {
            ...
            unsafe { T::entity(&mut accessor, entity) }
        })
    }

    type ChunksWith<'t, T: accessor::Chunked<A> + 't> = impl Iterator<Item = T::Chunk<'t>> + 't
    where
        Self: 't;
    fn chunks_with<T: accessor::Chunked<A>>(&self, mut accessor: T) -> Self::ChunksWith<'_, T> {
        ...
        self.chunks().map(move |chunk: entity::TempRefChunk<A>| {
            ...
            unsafe { T::chunk(&mut accessor, chunk) }
        })
    }
}
